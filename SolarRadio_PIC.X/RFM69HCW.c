...
byte send(RFM69* radio, byte msg) {
    ...
    radio->sendIndex++;
    if (radio->sendIndex == LEN_MESSAGE_BOOK) {
        radio->sendIndex = 0;
    }
    byte temp = SSP1BUF;
    if (radio->sendIndex == 0) {
        radio->received[LEN_MESSAGE_BOOK - 1] = temp;
    } else {
...
void setNodeAddress(RFM69* radio) {
    byte msg[] = {NODE_ADDRESS | WRITE, radio->node_address};
    sendBulk(radio, msg, 2);
}
...
void setModulationThreshold(RFM69* radio) {
    byte msg[] = {FIFO_THRESH | WRITE, radio->packet_length -1}; //packet length -1 and set MSb to 0
    sendBulk(radio, msg, 2);
}
